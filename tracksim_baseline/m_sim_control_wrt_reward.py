# TRACK SIMULATION
#   - naive control (ver2 - choose best reward)
...
if __name__ == "__main__":
    ...
    horizon_ev = 10  # Horizon of trajectory

    # Set reward parameters
    dev_rad_th = math.pi * 30 / 180  # Rad threshold
    dist_cf_th = 20  # Distance center-front threshold
    w_max = np.max(u_w_set)  # Maximum angular-velocity

    # reward weights
    # outside, collision, dev_rad, dev_dist, dist_cf, linear_velocity, angular_velocity, reach_goal (dim = 8)
    r_weight = np.array([-10, -10, 0.5 / (dev_rad_th * dev_rad_th), 0.5 / 0.25, 0.2 / (dist_cf_th * dist_cf_th), 0.33 / (v_ref * v_ref),
                         0.33 / (w_max * w_max), 0], dtype=np.float32)

    sim_control.set_reward_components(r_weight, dev_rad_th, dist_cf_th, w_max)

    # SET SCREEN ------------------------------------------------------------------------------------------------------#
    if VIEW_SCREEN == 1:
        if MODE_SCREEN == 0:
            screen_alpha = sim_track.screen_alpha_wide
            screen_size = (sim_track.screen_size_wide[0], sim_track.screen_size_wide[1])
        else:
            screen_alpha = sim_track.screen_alpha_narrow
            screen_size = sim_track.screen_size_narrow
    ...
    for nidx_t in range(t_init, t_max):
        ...
        # STEP2: FIND CONTROL -----------------------------------------------------------------------------------------#
        traj_sel_ev, traj_array_ev, r_array_ev = sim_control.find_control_wrt_r(u_set, horizon_ev, data_ov_near, nidx_t)
        ...
        if VIEW_SCREEN == 1:
            ...
            sim_screen.draw_target_vehicle_arrow(data_ev_cur[1], data_ev_cur[2], data_ev_cur[3], data_ev_cur[6],
                                                 data_ev_cur[5], data_ev_cur[4], sim_control.v_range[1],
                                                 get_rgb("White"))
            ...
            sim_screen.draw_vehicle_border_trajectory_array(traj_ov_near_plot, size_traj_ov_near_plot, 5, 1.5, get_rgb("Maroon"))

            sim_screen.draw_trajectory_array_w_cost(traj_array_ev, r_array_ev, [], 0, 1)
